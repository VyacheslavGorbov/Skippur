...
class Employee_Availabilities extends Model
{
    ...
    public function AllAvailabilitiesForSite($employee_id, $e_availability_date){
	...
	function getCustomer($site_id, $e_availability_date){
    	$sql = 'SELECT * FROM Customers WHERE site_id = :site_id AND e_availability_date = :e_availability_date';
        $stmt = self::$_connection->prepare($sql);
